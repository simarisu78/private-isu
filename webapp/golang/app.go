...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	err := db.Select(&results, "SELECT p.id AS id, `user_id`, `body`, `mime`, p.created_at AS created_at FROM `posts` AS p JOIN users AS u ON p.user_id = u.id WHERE u.del_flg = 0 ORDER BY `created_at` DESC LIMIT 20")
	if err != nil {
		log.Print(err)
		return
	}
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	results := []Post{}
	err = db.Select(&results, "SELECT p.id, p.user_id, p.body, p.mime, p.created_at FROM `posts` AS p JOIN users AS u ON p.user_id = u.id WHERE p.created_at <= ? AND u.del_flg = 0 ORDER BY p.created_at DESC LIMIT 20", t.Format(ISO8601Format))
	if err != nil {
		log.Print(err)
		return
	}
...
func postIndex(w http.ResponseWriter, r *http.Request) {
	...
	filePath := fmt.Sprintf("../public/img/%d.%s", pid, ext)
...
func getImage(w http.ResponseWriter, r *http.Request) {
	...
	if ext == "jpg" && post.Mime == "image/jpeg" ||
		...
		filePath := fmt.Sprintf("../public/img/%d.%s", post.ID, ext)
		f, err := os.Create(filePath)
		if err != nil {
			log.Print(err)
			return
